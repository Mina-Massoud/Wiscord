...
const AI_TOOLS: Tool[] = [
  {
    functionDeclarations: [
      ...
      },
      {
        name: 'createNote',
        description:
          "Save a new note doc to the user's notes lab. Call this whenever the user asks you to write a plan, summary, study guide, checklist, breakdown, or any long-form structured content they want to keep. Creates a fresh notes doc and surfaces it as a clickable pill in the chat; the user can open it in the editor or delete it from /app/labs/notes. Runs immediately, no confirmation. Write `markdown` in your normal genz voice. Use real markdown STRUCTURE — `# Heading`, `## Subheading`, `- bullet`, `1. ordered`, `> blockquote`, fenced code blocks. DO NOT use `**bold**`, `*italic*`, or `[links](url)` — those don't render correctly in v1 and surface as literal characters. Use headings and list nesting for emphasis instead. Typical length 150–600 words; longer only if the user explicitly asks for depth. Don't pad — a tight, well-structured plan beats a wordy one.",
        parameters: {
          type: Type.OBJECT,
          properties: {
            title: {
...
export async function* streamPersonalAnswer(args: StreamArgs): AsyncGenerator<StreamEvent> {
  ...
  // Diagnostic — fires once per stream. Empty completions
...
function synthesizeFinishReasonMessage(reason: string): string {
  switch (reason) {
    ...
    case 'UNEXPECTED_TOOL_CALL':
      return "Tool call came back malformed — try again or rephrase what you want done.";
    case 'LANGUAGE':
