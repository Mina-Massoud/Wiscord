...
import type { FakeAnnouncement } from '@/data/fake-shell.types';
 ...
 * not routed.
 */
export function AnnouncementBanner({ announcement }: AnnouncementBannerProps): React.JSX.Element {
  return (
    <article className="group bg-glass-surface-2 border-glass-border hover:border-glass-border-strong relative flex items-center gap-4 overflow-hidden rounded-lg border pr-4 pl-5 transition-colors">
      ...
      <div className="relative flex min-w-0 flex-1 flex-col gap-1 py-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-ink text-control truncate font-semibold">{announcement.headline}</p>
        </div>
        <p className="text-ink-muted text-caption truncate">{announcement.tagline}</p>
      </div>
